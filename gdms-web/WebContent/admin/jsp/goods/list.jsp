...
<script type="text/javascript">
	...
	function baocun(a){
		...
		//取得所有参数必须参数（修改项+主键）
		var $tr = $(a).parents("tr");
		var goodside  = $tr.children("td:eq(0)").children("input").val();//商品id
		...
		var amout  = $tr.children("td:eq(5)").children("input").val();//商品数量
		
		var param = {"goodside":goodside, "name":name,  "spec":spec,
					 "price":price,  "amout":amout};
		...
		$.ajax({
			...
			success:function(r){
				...
				$tr.children("td:eq(5)").html(amout);
				$(a).hide();//修改/保存按钮的切换
				$(a).prev("a").show();
